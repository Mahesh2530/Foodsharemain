...
import com.klef.sdp.backendproject.service.UserDonationService;
...
@RestController
@RequestMapping("/api/v1/donation-requests")
@CrossOrigin("*")
public class UserDonationController {
    ...
    public ResponseEntity<?> createDonationRequest(@RequestBody DonationRequestRequestDTO request) {
        try {
            DonationRequestResponseDTO response = userDonationService.createDonationRequest(request);
    ...
    public ResponseEntity<?> cancelDonationRequest(@PathVariable String requestId, @RequestBody Map<String, String> request) {
        try {
            ...
            String response = userDonationService.cancelDonationRequest(requestId, userId);
    ...
    public ResponseEntity<?> listDonationRequests(@RequestParam(required = false) String userId,
        ...
        try {
            List<DonationRequestResponseDTO> response = userDonationService.listDonationRequests(userId, status);
    ...
    public ResponseEntity<?> approveDonationRequest(@PathVariable String requestId, @RequestBody Map<String, String> request) {
        try {
            ...
            DonationRequestResponseDTO response = userDonationService.approveDonationRequest(requestId, userId);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            System.out.println(e.getMessage());
            return ResponseEntity.status(400).body("Invalid Request: " + e.getMessage());
